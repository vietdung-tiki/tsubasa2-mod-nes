...
function CheckNesHex() {
  ...
  document.getElementById('checknes').innerText =
    ' type:' +
    NesHex[0].toString(16) +
    ' ' +
    NesHex[1].toString(16) +
    ' ' +
    NesHex[2].toString(16);
  checkramtype(NesHex);
  TeamSelectChange();
  showTeamEditbuttondiv(0); //队伍选择
  ...
  //BindCHRDataAndHTML();
  ChangeOrGetMusic(0);
  document.getElementById('MusicTeamNameList')[0].selected = true;
  getTeamMusicVal(); //音乐初始化完毕执行
  LoadHex16(); //16进制编辑器
  ...
  $('#playerGKAbilitySe').val('0');
  GetPlayerData(1); //GK能力
  LoadSkills(); //必杀
  Changeskilladdtype();
  ...
  showCHRbuttondiv(0);
  GetBGview();
  $('#BGview_ID_0').get(0).selectedIndex = 0x17;
  GetBGview_BG(-1);
  It0528BG();
  GetBGview_0528_05cc_list(); //CHR选项+背景
  setTimeout(function () {
