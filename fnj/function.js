...
function ChangeTeam() {
  if (isaddTeam == false) {
    ...
    if (阵型addr != 0) {
      NesHex[阵型addr] =
        $('#Team_DF').get(0).selectedIndex +
        $('#Team_AK').get(0).selectedIndex * 0x10;
    }
    ...
    if (datas.length <= addteamdeflen || newaddr == false) {
      ...
      var defaddr =
        Team_Formation + $('#Add_Team_Select').get(0).selectedIndex * 2;
      var dz = 0;
      var 开关索引1 = 0;
      var 开关索引2 = 0;
      if (IsCn) {
    ...
    alertMsg('#isfileload', 'green', 'Star player added successfully~');
    setTimeout(function () {
...
function Add_TeamSelectChange() {
  ...
  $('#addteamStr').html(
    addteam_temp1 + addteam_temp2 + '<br>' + addteam_temp3 + ' 0F',
  );
}
...
function TeamSelectChange() {
  ...
  try {
    var selectitems = document
      .getElementById('playerlistdiv')
      .getElementsByTagName('select');
    for (var i = 0, j = selectitems.length; i < j; i++) {
      selectitems[i].options[0].selected = true;
    }
    selectitems = document
      .getElementById('replayerlistdiv')
      .getElementsByTagName('select');
    for (var i = 0, j = selectitems.length; i < j; i++) {
      selectitems[i].options[0].selected = true;
    }
  ...
  if (selectindex == 0 || selectindex == 1 || selectindex == 2) {
    ...
    switch (selectindex) {
      ...
      case 2:
        ...
        for (var i = 0; i < 8; i++) {
          GetPlayerByData(
            '#selectplayert' + addPreZero2(i + 1, 2),
            替补bits + i,
          );
        }
    ...
    for (var i = 0; i < 11; i++) {
      if (i == 0) {
        ...
        GetPlayerByData(
          '#selectplayer' + addPreZero2(i + 1, 2),
          球员地址bits + i,
        );
      }
    }
...
function ChrSelectColor() {
  for (var i = 0; i <= 3; i++) {
    $('#ChrColor' + (i + 1)).css(
      ...
      $('#ChrColor' + (i + 1))
        .children()
        .eq($('#ChrColor' + (i + 1)).get(0).selectedIndex)
...
//填充Selectlist带字节长度参数
function fillSelectlist_n(Select_, i, n) {
  Select_.append(
    "<option value='" +
      i +
      "'>" +
      addPreZero2(i.toString(16).toUpperCase(), n) +
      '</option>',
  );
}
//填充Selectlist默认填充转16禁止的2位字符串
function fillSelectlist(Select_, i) {
  Select_.append(
    "<option value='" +
      i +
      "'>" +
      addPreZero(i.toString(16).toUpperCase()) +
      '</option>',
  );
}
//填充Selectlist 全自定
function fillSelectlist_x(Select_, x, y) {
  Select_.append("<option value='" + x + "'>" + y + '</option>');
}
//拼接option字符串
...
//拼接option字符串 指定长度按10进制字符串
function fillSelectlist_S_10(str, len) {
  for (var i = 0; i < len; i++) {
    str += '<option value= "' + i + '">' + i + '</option>';
  }
  return str;
...
function 添加几率文本(动作, select) {
  ...
  for (var i = 0; i < temp.length; i++) {
    $(select).append(
      $('<option/>').text(toHex16(temp[i])).attr('value', toHex16(temp[i])),
    );
  }
}
...
function alertMsg(id, color, txt, time = 2500, type = 0) {
  if (type == 0) {
    ...
    $('#msgboxdiv').css(
      'left',
      ($(window).width() - $('#msgboxdiv').width()) / 2 + 'px',
    );
    $('#msgboxdiv').show();
...
function Team_SC_Change() {
  ...
  document.getElementById('Team_SC_C0')[
    NesHex[Team_Sock_Col_addr + selectindex * 2 + 0]
  ].selected = true;
  document.getElementById('Team_SC_C1')[
    NesHex[Team_Sock_Col_addr + selectindex * 2 + 1]
  ].selected = true;
  Team_SC_Change2();
  $('#Team_SC_str').html(
    '0x' + toHex16(Team_Sock_Col_addr + selectindex * 2, 5),
  );
}
function CG_Team_SC() {
  var selectindex = $('#Team_SC_Select').get(0).selectedIndex;
  NesHex[Team_Sock_Col_addr + selectindex * 2 + 0] =
    $('#Team_SC_C0').get(0).selectedIndex;
  NesHex[Team_Sock_Col_addr + selectindex * 2 + 1] ==
    $('#Team_SC_C1').get(0).selectedIndex;
  alertMsg('#isfileload', 'green', 'Sock color updated successfully~');
}
...
function Team_SC_Change2() {
  ...
  setTimeout(function () {
    ...
    CGimg.src = document.getElementById('canteamscpict').src =
      CGcanvas.toDataURL('image/png');
  }, 200);
}
