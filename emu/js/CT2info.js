// 添加新的常量定义到文件开头
const MATCH_PERIODS = [
  { value: '00', text: 'First Half' },
  { value: '01', text: 'Second Half' },
  { value: '02', text: 'Extra Time (1st Half)' },
  { value: '03', text: 'Extra Time (2nd Half)' },
  { value: '04', text: 'Penalty Shootout' },
  { value: '05', text: 'Watch Ending' },
];

const bollpostion = [
  { value: '00', text: 'Ground' },
  { value: '01', text: 'Penalty Area (Low Ball)' },
  { value: '02', text: 'Penalty Area (High Ball)' },
];
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function createCoordSelector(player, stats, coordSpan) {
    ...
    saveButton.addEventListener('click', function () {
      const baseAddr =
        player.index < 11
          ? 0x300 + player.index * 0x0c
          : 0x384 + (player.index - 11) * 0x0c;
  ...
  function getPlayerStats(index) {
    // 计算基础地址
    const baseAddr =
      index < 11 ? 0x300 + index * 0x0c : 0x384 + (index - 11) * 0x0c;
    const nes = db.nes;
    ...
    li.addEventListener('click', function (e) {
      ...
      statsDiv.innerHTML = `
                <div style="margin: 5px 0; padding: 8px; background: #f5f5f5; border-radius: 4px; font-size: 0.9em;">
                    <div style="margin-bottom: 8px;">
                        ...
                        ">
                            #${player.position}${
        db.nes.ram[0x441] === player.index ? '✓ 持球中' : '让我持球'
      }
                        </button>
                    </div>
                    <b>详细数据:</b><br>
                    体力值: <span style="color:${
                      stamina > 30000 ? 'green' : 'red'
                    }">${stamina}</span>
                    ...
                    <select class="level-select" style="margin-left:5px;">
                        ${Array.from(
                          ...
                          (_, i) =>
                            `<option value="${i}" ${
                              stats.level === i ? 'selected' : ''
                            }>${i}</option>`,
                        )}
                    </select><br>
                    坐标: <span class="coord-text" style="cursor:pointer;text-decoration:underline;">(${
                      stats.xPos
                    }, ${stats.yPos})</span><br>
                    ${
                      isGK
                        ? `
                        硬直: <span class="gk-stiff" style="cursor:pointer">${stats.stiff
                          .toString(16)
                          .toUpperCase()}${
                            stats.stiff === 6 ? '(空门)' : ''
                          }</span><br>
                        ...
                        状态: <span class="gk-status" style="cursor:pointer">${
                          stats.status === 0x4f
                            ? '摔倒'
                            : stats.status === 0x1a
                            ? '漏球'
                    ...
                    `
                        : `
                        硬直: <span class="player-stiff" style="cursor:pointer">${stats.stiff
                          .toString(16)
                          .toUpperCase()}</span>
                    `
                    }
      ...
      if (isGK) {
        ...
        gkStiff.addEventListener('click', (e) => {
          ...
          gkStiff.textContent = `${newStiff.toString(16).toUpperCase()}${
            newStiff === 6 ? '(空门)' : ''
          }`;
        });
  ...
  function createFloatingDialog(teamA, teamB) {
    ...
    title.innerText = 'Match Information Enhanced Edition';
    titleBar.appendChild(title);

    // 添加图钉按钮
    var pinButton = document.createElement('button');
    pinButton.innerText = '📌Click me to lock';
    ...
    pinButton.addEventListener('click', function () {
      ...
      pinButton.innerText = isPinned ? '📌Locked' : '📌Click me to lock';
    });
    ...
    const tabs = [
      { id: 'matchInfo', text: 'Match Information' },
      { id: 'playerInfo', text: 'Player Information' },
    ];
  ...
  function createMatchPanel() {
    ...
    timeContainer.innerHTML = `
            <div style="display:flex;align-items:center;margin-bottom:10px">
                <span style="margin-right:10px">Game time:</span>
                <select id="matchTime" style="width:120px">
                    ${gametime.map(
                      (time, i) =>
                        `<option value="${i}" ${
                          db.nes.ram[0x5f7] === i ? 'selected' : ''
                        }>${time}</option>`,
                    )}
                </select>
            </div>
    ...
    periodContainer.innerHTML = `
            <div style="display:flex;align-items:center;margin-bottom:10px">
                <span style="margin-right:10px">Competition Stage:</span>
    ...
    stageContainer.innerHTML = `
            <div style="display:flex;align-items:center;margin-bottom:10px">
                <span style="margin-right:10px">Competition Level:</span>
        <select id="matchStage" style="width:200px">
            ${Array.from({ length: 256 }, (_, i) => {
              ...
              const displayText =
                i < teamlist.length
                  ? `${hexValue}-${teamlist[i]}`
                  : `${hexValue}-Unknown level`;
              return `<option value="${i}" ${
                db.nes.ram[0x26] === i ? 'selected' : ''
              }>${displayText}</option>`;
            })}
        </select>
            </div>
    ...
    const tacticsOptions = [
      { value: '00', text: 'Common' },
      { value: '01', text: 'Pressing' },
      { value: '02', text: 'Counterattack' },
      { value: '03', text: '03' },
      { value: '04', text: '04' },
      { value: '05', text: '05' },
      { value: '06', text: '06' },
    ...
    formationsDiv.innerHTML = `
            <div style="margin-bottom:10px">
                <div style="display:flex;align-items:center;margin-bottom:10px">
                    <span style="margin-right:10px">Our formation:</span>
                    <select id="formation-A" style="width:80px">
                        ${formationOptions
                          .map(
                            (opt) =>
                              `<option value="${opt.value}" ${
                                db.nes.ram[0x2c] === parseInt(opt.value, 16)
                                  ? 'selected'
                                  : ''
                              }>${opt.text}</option>`,
                          )
                    ...
                    <span style="margin:0 10px">Tactics:</span>
                    <select id="tactics-A" style="width:80px">
                        ${tacticsOptions
                          .map(
                            (opt) =>
                              `<option value="${opt.value}" ${
                                db.nes.ram[0x2d] === parseInt(opt.value, 16)
                                  ? 'selected'
                                  : ''
                              }>${opt.text}</option>`,
                          )
                ...
                <div style="display:flex;align-items:center;margin-bottom:10px">
                    <span style="margin-right:10px">Oponent Formation:</span>
                    <select id="formation-B" style="width:80px">
                        ${formationOptions
                          .map(
                            (opt) =>
                              `<option value="${opt.value}" ${
                                db.nes.ram[0x2e] === parseInt(opt.value, 16)
                                  ? 'selected'
                                  : ''
                              }>${opt.text}</option>`,
                          )
                    ...
                    <select id="tactics-B" style="width:80px">
                        ${tacticsOptions
                          .map(
                            (opt) =>
                              `<option value="${opt.value}" ${
                                db.nes.ram[0x2f] === parseInt(opt.value, 16)
                                  ? 'selected'
                                  : ''
                              }>${opt.text}</option>`,
                          )
    ...
    teamNameContainer.innerHTML = `
            <div style="margin-bottom:10px">
                <div style="display:flex;align-items:center;margin-bottom:10px">
                    <span style="margin-right:10px">Our Team ID</span>
                ...
                <div style="display:flex;align-items:center;margin-bottom:10px">
                    <span style="margin-right:10px">Oponent Team ID:</span>
    ...
    ballpsContainer.innerHTML = `
            <div style="margin-bottom:10px">
                <div style="display:flex;align-items:center;margin-bottom:10px">
                    <span style="margin-right:10px">Football Status:</span>
                    <select id="ballps" style="width:80px">
                 ${bollpostion
                   .map(
                     (opt) =>
                       `<option value="${opt.value}" ${
                         db.nes.ram[0x044e] === parseInt(opt.value, 16)
                           ? 'selected'
                           : ''
                       }>${opt.text}</option>`,
                   )
    ...
    panel.querySelectorAll('select').forEach((select) => {
      select.addEventListener('change', function () {
        let value;
        if (
          this.id.startsWith('formation-') ||
          this.id.startsWith('tactics-') ||
          this.id === 'homeTeamId' ||
          this.id === 'awayTeamId'
        ) {
  ...
  function createPlayerPanel(teamA, teamB) {
    ...
    teamA.forEach((player) =>
      listA.appendChild(createPlayerListItem(player, teamAContainer)),
    );
    ...
    teamBTitle.textContent = 'Oponent players';
    ...
    teamB.forEach((player) =>
      listB.appendChild(createPlayerListItem(player, teamBContainer)),
    );
  ...
  function switchTab(selectedTab) {
    ...
    document.querySelectorAll('[data-tab-id]').forEach((tab) => {
      tab.style.borderBottom =
        tab === selectedTab ? '2px solid #4CAF50' : 'none';
    });
