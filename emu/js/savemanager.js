...
(function () {
  ...
  function openDB(callback) {
    ...
    request.onupgradeneeded = function (e) {
      ...
      if (!db.objectStoreNames.contains(CONFIG_STORE)) {
        let configStore = db.createObjectStore(CONFIG_STORE, {
          keyPath: 'key',
        });
  ...
  function updateSaveSlot(slot, screenshot, timestamp, callback) {
    init(function () {
      ...
      let record = {
        slot:
          slot === AUTO_SAVE_SLOT ? `${AUTO_SAVE_SLOT}_${loadedName}` : slot, // 自动存档与游戏名绑定
  ...
  function getAllSaveSlots(callback) {
    init(function () {
      ...
      req.onsuccess = function () {
        let result = req.result.filter(
          (item) =>
            item.slot !== `${AUTO_SAVE_SLOT}_${loadedName}` ||
            item.romName === loadedName,
        ); // 仅保留当前游戏的自动存档
  ...
  function showAutoSavePrompt(record, callback) {
    ...
    img.style.cssText =
      'width: 256px; height: 240px; object-fit: contain; margin: 10px 0;';

    let text = document.createElement('p');
    text.textContent = `Auto save at (${record.timestamp})，Load？`;
...
(function () {
  function showGameManager() {
    SaveManager.getAllSaveSlots(function (err, list) {
      ...
      list.forEach((item) => {
        ...
        if (!gameMap[item.romName])
          gameMap[item.romName] = { size: 0, count: 0 };
        gameMap[item.romName].size += item.saveState
          ? JSON.stringify(item.saveState).length
          : 0;
      ...
      Object.keys(gameMap)
        ...
        .forEach((name) => {
          ...
          row.style =
            'display:flex;align-items:center;justify-content:space-between;padding:0.3em 0;border-bottom:1px solid #eee;';
          row.innerHTML = `<span>${name}</span>
                    <span style="font-size:0.9em;color:#888;">${(
                      gameMap[name].size / 1024
                    ).toFixed(1)} KB</span>
  ...
  function createUI() {
    ...
    SaveManager.getAllSaveSlots(function (err, list) {
      ...
      if (list && list.length) {
        total = list.reduce(
          (sum, item) =>
            sum + (item.saveState ? JSON.stringify(item.saveState).length : 0),
          0,
        );
      }
      document.getElementById('saveTotalSize').textContent = `(${(
        total /
        1024 /
        1024
      ).toFixed(1)} mb)`;
  ...
  function refreshSaveList() {
    SaveManager.getAllSaveSlots(function (err, list) {
      ...
      // 自动存档
      let autoSave = list.find(
        (item) => item.slot === `${SaveManager.AUTO_SAVE_SLOT}_${loadedName}`,
      );
      let autoScreenshot =
        autoSave && autoSave.romName === loadedName ? autoSave.screenshot : '';
      let autoTimestamp =
        autoSave && autoSave.romName === loadedName
          ? autoSave.timestamp
          : '<empty>';
      saveList.innerHTML += `
                <div class="auto-save-row" style="padding:0.5rem;border-bottom:1px solid #ccc;background:#f0f0f0;">
                    <div style="display:flex;align-items:center;justify-content:space-between;">
                        ...
                        <div>
                            <button class="load-btn" data-slot="${SaveManager.AUTO_SAVE_SLOT}_${loadedName}">Load</button>
                        </div>
                    </div>
                </div>`;
      ...
      // 20个普通存档
      for (let i = 0; i < 20; i++) {
        let slotSave = list.find(
          (item) => item.slot === i && item.romName === loadedName,
        );
        let screenshot = slotSave ? slotSave.screenshot : '';
        let timestamp = slotSave ? slotSave.timestamp : '<empty>';
        saveList.innerHTML += `
                    <div data-slot="${i}" style="padding:0.5rem;border-bottom:1px solid #ccc;cursor:pointer;display:flex;align-items:center;justify-content:space-between;">
                        ...
                        <div style="display:flex;gap:0.2rem;">
                            ...
                            <button class="load-btn" data-slot="${i}">Load</button>
                        </div>
                    </div>`;
      }
      ...
      Array.from(saveList.querySelectorAll('div[data-slot]')).forEach((div) => {
        div.onclick = function (e) {
          ...
          switchSlot(s);
          Array.from(saveList.querySelectorAll('div[data-slot]')).forEach(
        ...
        btn.onclick = function (e) {
          ...
          SaveManager.getSaveData(realSlot, function (err, record) {
            ...
            if (record && record.saveState) {
              ...
              if (
                String(slot) === `${SaveManager.AUTO_SAVE_SLOT}_${loadedName}`
              ) {
                log('已加载自动存档', 'save');
              } else {
                log('已加载存档：' + slot, 'save');
  ...
  function updateCurrentSlotDisplay() {
    ...
    if (disp) {
      disp.textContent =
        'Current card slot:' +
        (window.currentSaveSlot !== undefined ? window.currentSaveSlot : 0);
    }
  }
